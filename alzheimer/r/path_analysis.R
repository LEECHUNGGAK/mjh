...
work_data <- ori_work_data %>% 
    ...
    setNames(., c("id", "registration_year", "age", "gender", "weight",
                  ...
                  "drinking_total", "num_of_workouts_per_week", "htn",
    ...
    mutate(age = as.integer(age),
           ...
           num_of_workouts_per_week = as.double(num_of_workouts_per_week),
...
model <- "
breast_ca ~ age + htn + diabetes + cardiac_disease + dyslipidemia
dementia ~ breast_ca + breast_ca_ctx + age + bmi + smoking_total + drinking_total
"

result <- sem(model = model,
              data = work_data)

semPaths(result, "std", style = "lisrel")
